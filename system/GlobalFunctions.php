 ...
 * @LastEditors: Wibus
 * @LastEditTime: 2021-08-29 16:44:36
 * Coding With IU
 */
...
function Autofirst(){
    ...
    $sm=array_filter(array_merge(array_unique($arrMail)));
    ...
    for($i=0;$i<count(array_unique($su));$i++){
        if($su[$i]!=="" && $num<16){
            ...
            <img alt="'.$arrAuthor1[0].'" src="https://gravatar.loli.net/avatar/' . md5(strtolower($sm[$i])) . '?d=wavatar" class="avatar avatar-42 photo" height="42" width="42" /></a>
         </li> 
         ';
        }
    }
